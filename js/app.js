...
const authorText=c=>isAdmin()?`<small class="record-author">أضافها: ${esc(c.createdByName||'غير مسجّل (سجل قديم)')}${c.lastEditedByName?' · آخر حفظ: '+esc(c.lastEditedByName):''}</small>`:'';
const costText=n=>!isAdmin()?'غير متاح':hideCosts?'••••••':money(n)+' ل.س';
...
async function loadData(){
 try{
  ...
  reportDraftCache=isAdmin()?await api('/report-draft'):{};
...
function setCostHidden(v){if(!isAdmin())return;hideCosts=v;localStorage.setItem(HIDE_KEY,v?'1':'0');renderAll();toast(v?'تم إخفاء التكاليف والإيرادات':'تم إظهار التكاليف والإيرادات')}
function showPage(page,focus=false){
 if(page==='reports'&&!isAdmin())return;
...
function printReport(){if(!isAdmin())return;
  saveReportDraft();
...
function applyPermissions(){
 document.querySelectorAll('.revenue-card,#homeSummary,#costs,#invoiceCostToggle,#backup,[data-page="reports"],[data-action="print"]').forEach(el=>el.classList.toggle('hidden',!isAdmin()));
 for(const id of ['pc','lc']) $(id)?.closest('label')?.classList.toggle('hidden',!isAdmin());
 $('total')?.closest('.total')?.classList.toggle('hidden',!isAdmin());
}
function renderAll(){renderHome();renderCars();renderCustomers();renderInvoices();renderReports();updateCostButtons();if(currentUser)renderSettings();applyPermissions()}
function uniqueCustomers(){return new Map(cars.map(c=>[(c.owner||'').trim().toLowerCase(),c])).size}
function renderHome(){
 const rev=cars.reduce((s,c)=>s+total(c),0);$('customersCount').textContent=uniqueCustomers();$('carsCount').textContent=cars.length;$('invoicesCount').textContent=cars.length;$('revenue').textContent=!isAdmin()?'':hideCosts?'••••••':money(rev)+' ل.س';
 $('recent').innerHTML=cars.slice(0,6).map(c=>`<button class="recentrow" data-action="openCar" data-id="${esc(c.id)}"><span><b>${esc(c.plate)}</b><small>${esc(c.owner||'بدون اسم')} · ${esc(c.phone||'لا يوجد هاتف')}</small>${authorText(c)}</span><strong>${esc(c.make||'Mercedes')} ${esc(c.model||'')}</strong><em>${esc(c.invoiceNo||'-')}</em></button>`).join('')||'<div class="empty-state">لا توجد سيارات مسجلة بعد.</div>';
 const avg=cars.length?Math.round(rev/cars.length):0, withPhone=cars.filter(c=>c.phone).length;
 $('homeSummary').innerHTML=`<div><span>متوسط الفاتورة</span><b>${!isAdmin()?'':hideCosts?'••••••':money(avg)+' ل.س'}</b></div><div><span>ملفات فيها هاتف</span><b>${withPhone}</b></div><div><span>آخر إضافة</span><b>${cars[0]?esc(cars[0].date):'لا يوجد'}</b></div>`;
}
...
function renderCars(){
 ...
 box.innerHTML=found.map(c=>`<article class="car"><div class="car-top"><div><h3>🚘 ${esc(c.make||'Mercedes')} ${esc(c.model||'')}</h3><span class="invoice-badge">${esc(c.invoiceNo||'بدون فاتورة')}</span>${authorText(c)}</div><b class="plate">${esc(c.plate)}</b></div><div class="vin">VIN: ${esc(c.vin)}</div><div class="car-grid"><div><span>العميل</span><b>${esc(c.owner||'-')}</b></div><div><span>الهاتف</span><b>${esc(c.phone||'-')}</b></div><div><span>السنة</span><b>${esc(c.year||'-')}</b></div><div><span>العداد</span><b>${c.odo?money(c.odo)+' كم':'-'}</b></div><div><span>تاريخ التسجيل</span><b>${esc(c.date||'-')}</b></div>${isAdmin()?`<div><span>الإجمالي</span><b>${costText(total(c))}</b></div>`:''}</div><div class="parts-title">القطع والأعمال</div><div class="chips">${partsHtml(c.parts)}</div><div class="actions"><button data-action="edit" data-id="${esc(c.id)}">✎ تعديل</button><button class="${isAdmin()?'':'hidden'}" data-action="print" data-id="${esc(c.id)}">🖨 الفاتورة</button><button data-action="delete" data-id="${esc(c.id)}">حذف</button></div></article>`).join('')||'<div class="panel empty-state">لا توجد ملفات مطابقة للبحث.</div>';
}
...
function renderInvoices(){
 ...
 box.innerHTML=found.map(c=>`<div class="inv"><div><b>${esc(c.invoiceNo||'-')}</b><strong>${esc(c.plate)}</strong><small>${esc(c.owner||'بدون اسم')} · ${esc(c.phone||'لا يوجد هاتف')} · VIN: ${esc(c.vin)}</small><small>${esc(c.make||'Mercedes')} ${esc(c.model||'')} · ${esc(c.date||'')}</small>${authorText(c)}</div><div class="inv-actions">${isAdmin()?`<strong>${costText(total(c))}</strong>`:''}<button class="${isAdmin()?'':'hidden'}" data-action="print" data-id="${esc(c.id)}">🖨 طباعة</button><button data-action="openCar" data-id="${esc(c.id)}">فتح الملف</button></div></div>`).join('')||'<div class="empty-state">لا توجد فواتير مطابقة للبحث.</div>';
}
function reportDraft(){return reportDraftCache||{}}
function saveReportDraft(){if(!isAdmin())return;
...
function renderReports(){if(!isAdmin())return;
